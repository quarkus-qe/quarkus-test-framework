...
public class OpenShiftHttpsIT {
    ...
    public void basicHttpsTest() {
        assertEquals(
                "Hello World!",
                // CN nor SAN in the certificate does not match OCP route, so we cannot verify HOST
                // FIXME if SAN for OCP route is added to the certificate, allow host verification
                app.mutinyHttps(false, null, true).get("/greeting").sendAndAwait().bodyAsString(),
